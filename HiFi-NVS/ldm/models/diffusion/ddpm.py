...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.first_stage_key = first_stage_key #"image_target"
        self.image_size = image_size  # try conv?
        self.channels = channels #4 channels?
        self.use_positional_encodings = use_positional_encodings
        self.model = DiffusionWrapper(unet_config, conditioning_key) #important
...
class LatentDiffusion(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config, # config for autoencoders
                 cond_stage_config, # 
                 depth_stage_config,
        ...
        self.instantiate_first_stage(first_stage_config) #instantiate self.first_stage_model, ldm.models.autoencoder.AutoencoderKL
        self.instantiate_cond_stage(cond_stage_config) #instantiate self.cond_stage_model,ldm.modules.encoders.modules.FrozenCLIPImageEmbedder
        self.cond_stage_forward = cond_stage_forward
        self.instantiate_depth_stage(depth_stage_config)

        # construct linear projection layer for concatenating image CLIP embedding and RT
        self.cc_projection = nn.Linear(772, 768) # single linear for projection
    ...
    def instantiate_depth_stage(self, config):
        model = instantiate_from_config(config)
        self.midas = model.eval()
        for param in self.first_stage_model.parameters():
            param.requires_grad = False
    ...
    def get_depth(self,x):
        ...
        return cc.repeat(1,3,1,1)
    
    # def viz_depth(self,x):
    #     depths = x.mean(1).unsqueeze(1).repeat(1,3,1,1).detach()
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x = super().get_input(batch, k) #image_target
        T = batch['T'].to(memory_format=torch.contiguous_format).float() #position of image_target
        ...
        x = x.to(self.device)
        x = self.get_depth(x).detach()
        encoder_posterior = self.encode_first_stage(x) 
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        cond_key = cond_key or self.cond_stage_key
        xc = super().get_input(batch, cond_key).to(self.device) #image_cond
        if bs is not None:
            xc = xc[:bs]
        cond = {}
        ...
        with torch.enable_grad():
            ...
            # using a linear layer to project 772 -> 768
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if isinstance(cond, dict): #hybrid means image and text condition?
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.diffusion_model = instantiate_from_config(diff_model_config) #ldm.modules.diffusionmodules.openaimodel.UNetModel
